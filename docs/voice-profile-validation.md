...
commands below use the bundled tool (`build/VoiceIsLocal.app/Contents/MacOS/voiceislocal`) or a `swift build`
one. The speaker models must be installed (`voiceislocal setup --speakers`, or Setup → Speaker labels).
Where a step says `<session>`, use the session ID or `.holos` path that `voiceislocal session list` shows.

## H15: A confirmed voice is suggested in a later meeting

1. Turn **Remember voices** on: `voiceislocal people remember on`, or check "Remember voices of people I
   name" in **People…** (menu bar). `voiceislocal people list` says `Remember voices: on`.
2. Meeting A: record a short meeting with the willing person (or import one with
   `voiceislocal session import <file>`). Once it is labelled, find their speaker with
   `voiceislocal speakers list <A> --turns`, then link it with voice learning:
   `voiceislocal speakers link <A> <speaker> new:<Name> --learn-voice`.
...
3. Meeting B with the same person: record or import it. Once it is labelled,
   `voiceislocal speakers list <B>` shows `suggestion: Maybe <Name> (0.xx)` on their speaker. The
   suggestion is not in `exports/transcript.md`, `.txt`, or `.json`.
4. Confirm it: `voiceislocal speakers link <B> <speaker> <Name>`; the speaker is named in B's exports.
5. Forget the person: **People…** → select them → **Forget <Name>…**, or
   `voiceislocal people forget <Name> --yes`. `voiceislocal speakers list <B>` no longer shows a suggestion, and
   the meetings keep the name they were given (`voiceislocal speakers list <A>`).
6. Label meeting B again (`voiceislocal session diarize <B> --force`): no suggestion comes back.
...
6. `voiceislocal people export --output ~/Desktop/people.json` writes names and sample details without
